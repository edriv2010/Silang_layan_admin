...
LoadPageObj = function (name, target, oncomplete) {
    $("#div-loading").html('Loading Page ' + name + '.. Please Wait..');
    $("#div-loading").show();
    $(target).html('');
    $.ajax({ url: name, type: "POST", dataType: 'html',
...
ExecuteNoLoadingWithParam = function (name, data, oncomplete, param) {
    $.ajax({
        url: name,
        data: data, dataType: 'html',
        type: "POST",
...
ExecuteFunction = function (name, data, oncomplete) {
    $("#div-loading").html('Execute ' + name + '.. Please Wait..');
    $("#div-loading").show();
    $.ajax({
        url: name,
...
TableCheckIDFromSpanAndInput = function (TableID, CheckString, ColumnIndex, ExcludeObj) {
    var i1 = 0;
    var IsFound = false;
    $("#" + TableID + " tr").each(function () {
        var i2 = 0;
        $(this).find('td').each(function () {
            if (i2 == ColumnIndex && !IsFound) {
                ...
                $(this).find('input').each(function () {
                    if ($(this).val() == CheckString && !IsFound && !$(this).is(ExcludeObj)) {
                        IsFound = true;
                        return false;
                    };
                });
...
TableCheckIDFromSpan2 = function (TableID, CheckString, ColumnIndex, ExcludeObj) {
    var i1 = 0;
    var IsFound = false;
    $("#" + TableID + " tr").each(function () {
        var i2 = 0;
                ...
                $(this).find('td').each(function () {
                    if (i2 == ColumnIndex && !IsFoundColumn) {
                        ...
                        if (!IsFoundObject) {
                            ...
                            $(this).find('span').each(function () {
                                IsFoundObject = true;
                                if ($(this).attr("id") != undefined && $(this).attr("id").indexOf(ObjectID) != -1) {
                                    var t = $(this).text();
                                    ListItemID.push(t);
                                    return false;
                                }
